...
export function NeedHud({ character, onSelect }: Props) {
  const activeNeedDefs = NEED_DEFINITIONS.filter((def) => character.needs[def.id]);
  if (activeNeedDefs.length === 0) return null;

  return (
    <div className="fixed inset-x-0 z-10 flex justify-center px-3 [bottom:calc(0.75rem+env(safe-area-inset-bottom))]">
      <button
        onClick={onSelect}
